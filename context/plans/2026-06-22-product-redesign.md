...
- [~] Replace the watcher with the event-driven capture impl (behind a `capture` trait, mockable). **1.2a ✅ DONE** (`phase1/capture-seam`, D32): the `capture/` seam — `CaptureSource` trait + `FocusEvent` + `process_focus_event` (D8 exclusions) + `run` runner + `FakeCapture` (CI mock) + `PollingCapture` (interim, app-name+idle only — titles via AX in 1.2b); `watcher.rs` deleted; no IPC change. **1.2b TODO** (on-device): the event-driven macOS impl (NSWorkspace + `AXObserver` + browser URL + terminal cwd) + site parsing + project inference; flip `default_source()`.
