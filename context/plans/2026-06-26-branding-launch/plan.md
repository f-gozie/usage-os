...
**Created:** 2026-06-26 · **Status:** 🟢 **M1–M5 done — the landing is LIVE at [usageos.app](https://usageos.app) and the notarized Universal `v0.1.0` DMG is shipped on GitHub Releases.** Cloudflare Pages is git-integrated: push to `main` → auto-deploys; PRs get preview URLs. **Next = public launch — flip the repo to public (it's private), then the release tail (auto-update · Homebrew · social preview).** Newest handoff: `handoffs/2026-06-30-01-universal-dmg-shipped-public-next.md`. · **Owner:** Favour
...
- [~] **M2 — Landing** *(on the one Phase-5 branch)*: ✅ Astro static site in `landing/` (Product direction — pitch + framed app window, week + timeline crops, privacy, numbers; screen-time copy; live dial + week built client-side; theme switcher; **fonts bundled locally**). ✅ favicon + **OG card** (`public/og.png`) + meta. ✅ Builds clean (`astro build`); previews at `localhost:4321`. ✅ Deploy notes (`landing/README.md`). Replaced the old cyberpunk `landing/`. **Depth pass (2026-06-27, [D59](../../decisions.md)):** added a **categorization** section ("You decide what counts as work") + a calm **re-sort toggle**; converted the Privacy prose → real **exclusion rows**; cut the redundant stats band; "Deep work" → "Work"; a11y (focus-visible/aria-pressed/dial labels) + a phone breakpoint; timeline app-stretches newest-first. **Every public claim was verified against the code** — 3 were wrong and corrected across landing + README + in-app Settings (see D59). **Download wiring + width/copy pass (2026-06-30):** all three CTAs (nav/hero/final) now point straight at the universal DMG (`releases/latest/download/UsageOS-0.1.0-universal.dmg`, via a single `DMG_URL` const) with a plain detail line **"Universal · macOS 13+ · ~11 MB"** (owner: **no "notarized"** in copy, [[no-notarized-in-copy]]); container widened 1100 → **1220px** (side ink-frame kept); hero sub rewritten with the owner ("watches" → plain "keeps track of your screen time… See which apps and sites you spend the most time on", no em dash, categories not "work"). _The DMG link 404s until the repo is public — expected._ **Remaining:** **Cloudflare Pages deploy → `usageos.app`** (push to `main` auto-deploys); optional — embed Anton in the OG.
...
- [x] **M5 — Notarized DMG** ✅ **SHIPPED** — the **Universal (Intel + Apple Silicon), notarized** `UsageOS-0.1.0-universal.dmg` is live on the [v0.1.0 release](https://github.com/f-gozie/usage-os/releases/tag/v0.1.0), bundle id `com.usageos.app` ([D64](../../decisions.md) signing pipeline · [D65](../../decisions.md) universal + bundle id). _The release tail (auto-update · Homebrew · social preview) and the public-repo flip are still open._
  - [x] **Developer ID Application cert created** — none existed (Nudge is iOS); generated a local CSR → account-holder web portal → imported as a PKCS#12 with the Apple G2 intermediate. `codesign` signs unattended (full chain + hardened runtime + secure timestamp verified). The ASC API **can't** mint Developer ID certs (403, account-holder-only).
  - [x] **Notarization credential** — a dedicated App Store Connect **API key** (Developer role) as the `usageos-notarytool` keychain profile + Tauri's `APPLE_API_*` env. **No account secrets in the repo** (all in `~/.appstoreconnect/`, sourced by the release script; a from-source build is simply unsigned).
  - [x] **`tauri build` pipeline** — `scripts/release-macos.sh` (preflight → real sidecar → license notices → sign app + nested `usageos-ai` w/ hardened runtime → notarize → staple → DMG → verify). **Signing of the whole bundle verified working.**
  - [x] **`seed_db` bin → cargo example** — Tauri's bundler copies every package bin into the `.app`; the `perf`-gated seeder broke the bundle. Moved to `examples/` (`mainBinaryName` alone didn't fix it).
  - [x] **Third-party license notices** bundled — `cargo-about` (Rust, allowlist mirrors `deny.toml`) + npm prod tree + fonts OFL → `src-tauri/THIRD-PARTY-LICENSES.html`, committed + bundled as a resource (`scripts/gen-licenses.sh`).
  - [x] **`tauri.conf.json`** — `minimumSystemVersion 13.0` _(confirm)_, drag-to-Applications DMG layout, license resource.
  - [x] **Landing Download + Sponsor CTAs wired** — `releases/latest` + `github.com/sponsors/f-gozie`.
  - [x] **The notarized Universal DMG** — shipped on the release ([D65](../../decisions.md)). _Correction to the earlier "Apple Notary outage" framing: the first-ever notarization sat in Apple's first-submitter "additional analysis" queue ~24h — a known, documented behaviour (per Apple DTS), **not** an outage and **not** our signing/account. Once cleared, later submissions notarize in ~2 min._
  - [x] **Branded DMG background** ✅ **BUILT + notarized** *(2026-06-30, branch `feat/dmg-installer-background`)* — the installer was bare (drag worked, no backdrop art). Rendered `dmg-background.png` (660×400) + `@2x` (1320×800) from `design/dmg/dmg-background.html` (self-contained, fonts embedded) via headless Chrome, combined into a **retina multi-rep TIFF** (`src-tauri/dmg-background.tiff`, via `tiffutil -cathidpicheck`) and wired `bundle.macOS.dmg.background` to it. _(This Tauri version's bundler doesn't auto-combine a `@2x` PNG companion — the TIFF is the only way to get a crisp Retina background.)_ Held to the frozen design system: bone field `#EEEBE1`, **color only inside the Contexts dial** (wordmark is pure ink), **blue the one chrome accent** (the drag arrow). Top = dial mark + `USAGEOS`; a blue "drag → Applications" cue sits in the gap between the two real Finder icons (centers x=180/x=480, y=180, verified clear of both icons + their filename labels); bottom = quiet install line + version caption. The branded universal DMG was rebuilt via `scripts/release-macos.sh` → signed (Developer ID + hardened runtime) → **notarized + stapled (app *and* DMG)** → Gatekeeper-clean on a quarantined copy. _Remaining: swap onto the v0.1.0 release asset + re-point the tag._
  - [ ] **Owner ~1 click:** set the GitHub social preview to `landing/public/og.png`.
  - [x] **Auto-update — opt-in, ed25519-signed** *(2026-06-30, [D67](../../decisions.md); default decided by a 2-round [[debate]])* — `tauri-plugin-updater` (pinned `=2.9.0` to keep tauri on 2.9.x). Automatic checks **OFF by default**: onboarding "Updates" step recommends enabling, Settings "Software update" toggle + manual "Check now", launch check debounced once/24h. The check is a Rust-side fetch of a static `latest.json` (bypasses CSP — the sanctioned exception), sends only the version, no server/collection. `scripts/gen-latest-json.sh` + release-script step emit the signed `.app.tar.gz` + manifest. App-side green (fmt, clippy, 127 Rust, tsc, 32 vitest, bindings regen). **✅ Shipped:** the signed v0.1.0 rebuilt with the updater (DMG ~15 MB now — the HTTPS stack), notarized + stapled + Gatekeeper-clean; `UsageOS.app.tar.gz` + `latest.json` uploaded to the release; tag re-pointed. _(Updater key regenerated with a password mid-build — `--ci` keys aren't passwordless; pubkey + password in `~/.appstoreconnect/`. Allowed `CDLA-Permissive-2.0` for webpki-roots.)_
  - [x] **Homebrew cask** *(2026-06-30)* — personal tap **`f-gozie/homebrew-tap`** (`Casks/usageos.rb`, sha256 of the v0.1.0 DMG): `brew install --cask f-gozie/tap/usageos`. README updated. (Installs once the repo is public.)
  - [x] **CHANGELOG + version** *(2026-06-30)* — kept **v0.1.0** as the honest first *public* release (old prototype's 0.1.0 never shipped); real changelog entry added.
  - *(Then the tail: Sponsor link · social preview — owner 1-clicks.)*
  - **Also landed during M5 dogfood (capture/UI fixes on the running signed build):** the **Timeline shows window titles** instead of the always-"—" project ([D62](../../decisions.md)); **Chromium/Electron apps now capture titles + URLs** via `AXManualAccessibility` (Chrome/Claude were blank) ([D63](../../decisions.md)).
  - **Automation "Grant access" actually prompts now** *(2026-06-30, post-install dogfood on macOS 26, [D66](../../decisions.md))* — the button used the wildcard `AEDeterminePermissionToAutomateTarget(ask_user=true)` determination call, which doesn't raise the consent dialog (and an app isn't even listed under Settings → Automation until it scripts a *running* browser), so it silently no-opped. Now it sends a real Apple Event to each running browser to raise the prompt, and returns `NoBrowserRunning` so onboarding shows an "open your browser" hint instead of a dead-end. Gates green (fmt, clippy, 127 Rust, tsc, 32 vitest, bindings regenerated).
