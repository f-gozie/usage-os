...
- [ ] **M5 — Notarized DMG (+ release tail)** — the only thing left:
  - Developer-ID sign + notarize + DMG layout (drag-to-Applications); the owner has the Apple Developer cert (Nudge shipped), so unblocked.
  - **Build + sign the Swift sidecar:** the release pipeline must run `sidecar/build.sh` before `tauri build`, and the nested `usageos-ai` binary must be Developer-ID-signed + hardened-runtime + entitled (`sidecar/entitlements.plist`) — else notarization fails. (Discovered 2026-06-27: a placeholder sidecar stub had silently broken the on-device recap; the real binary is built locally now but is git-ignored, so the release build regenerates it.)
  - **Bundle third-party license notices** in the DMG (and optionally an About "Acknowledgements"): fonts are OFL-1.1 (license files already in `landing/public/fonts/`; the app bundles the same woff2 via `@fontsource` and needs them too) + npm + crates via `cargo-about` / `license-checker`. Use is legally fine (OFL/MIT/Apache, all permissive — `cargo-deny` confirms); the obligation is shipping the notices.
  - **Wire the landing Download + Sponsor CTAs** (currently dead `#`) to the DMG / GitHub Sponsors once they exist.
