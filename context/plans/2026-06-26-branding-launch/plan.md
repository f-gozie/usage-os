...
**Created:** 2026-06-26 · **Status:** 🟢 **M1–M4 done; the landing is LIVE at [usageos.app](https://usageos.app).** [PR #21](https://github.com/f-gozie/usage-os/pull/21) + [PR #22](https://github.com/f-gozie/usage-os/pull/22) merged to `main` (2026-06-28). Reviewed twice — gates green (incl. `--features perf`), 0 Critical (reports in `reviews/`); ADR [D59] in `decisions.md`. Cloudflare Pages is git-integrated: push to `main` → auto-deploys; PRs get preview URLs. **Next = M5 (notarized DMG + release tail).** Newest handoff: `handoffs/2026-06-28-01-landing-deployed-next-m5.md`. · **Owner:** Favour
...
- [~] **M5 — Notarized DMG (+ release tail)** — _signing + pipeline DONE & verified; the notarized DMG itself is blocked on an Apple Notary Service outage (see [D60](../../decisions.md)):_
  ...
  - [ ] **The notarized DMG** — ⏳ **Apple's Notary Service is down** (submissions stuck "In Progress" 5h+; a fresh one too). Pipeline is ready; re-run `scripts/release-macos.sh` once Apple recovers (or staple the in-flight submission). A signed (un-notarized) DMG works for local install today.
