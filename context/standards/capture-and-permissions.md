...
- Title change within an app: `AXObserver` for `kAXFocusedWindowChangedNotification` + `kAXTitleChangedNotification`. **`AXObserver`s are per-PID** — when the frontmost app changes, tear down the old observer and create a new one for the new PID, or title changes stop firing after the first switch. [capture-events-runloop.json gotchas]
...
| `accessibility-sys` | 0.2.0 (rel. 2025-03-22) | `AXObserver*`, focus/title notification constants, `kAXTitleAttribute`. Alternative/lower-level to the objc2 family — pick one in the spike. |
...
1. **Crate version + exact signatures.** A stale 2024 source claimed objc2 lacked `AXUIElement`; 0.3.2 docs contradict it. Confirm the *resolved* `Cargo.lock` version actually exposes `AXUIElement::new_application(pid) -> CFRetained<AXUIElement>` and `copy_attribute_value(&CFString, NonNull<*const CFType>) -> AXError` before relying on the snippets above. Also decide objc2 family vs `accessibility-sys` for the observer path. [capture-ax-titles.json riskyClaims]
2. **Electron / Chromium AX titles (HIGHEST RISK).** Chromium apps (Cursor, VS Code, Chrome, Brave) build their AX tree lazily and `AXManualAccessibility` can't be set externally. The top-level window `AXTitle` *usually* survives, but this is **unconfirmed by authoritative sources** for the project's most important apps (a code editor + browsers). Empirically read titles for Cursor, VS Code, Chrome, Brave, iTerm2 with only Accessibility granted; record real-title / empty / nil per app. If empty, the whole AX title approach for those apps is in question. [capture-ax-titles.json riskyClaims]
3. **Threading / run-loop model.** Both the `AXObserver` run-loop source and synchronously-delivered `NSNotificationCenter` blocks need a **running CFRunLoop**. `architecture.md`'s "AX calls run on the main thread" is imprecise — AX *queries* are callable off-main; what's constrained is where the observer source is serviced. Pick ONE model and prove callbacks fire without starving the Tauri UI: (a) attach to the main run loop via `run_on_main_thread`, or (b) a dedicated `std::thread` calling `CFRunLoopRun()`. Prove the per-PID observer is rebuilt on app switch. [capture-events-runloop.json riskyClaims]
4. **Apple Silicon (aarch64) build + runtime.** docs.rs renders these crates for x86_64 only (a rustdoc display artifact, not a restriction), but aarch64 runtime is unverified. Build and run the spike on a real Apple Silicon Mac (the mandated target per D9). [capture-events-runloop.json]
5. **Chromium `mode` readability (load-bearing for D8).** The sdef says `mode` "can be set only once during creation," which could mean creation-write-only. Confirm `mode of front window` is actually **readable** on an existing incognito window and returns the literal `"incognito"`. If not, the entire Chromium incognito-exclusion approach must change. Verify Arc's incognito semantics separately — its window model differs. [browser-automation-urls.json riskyClaims]
...
7. **osascript vs NSAppleScript latency.** No authoritative benchmark exists. Measure end-to-end one URL fetch (cold + warm); confirm it's fine for event-driven capture before deciding whether NSAppleScript's in-process speed is even needed. [browser-automation-urls.json]
